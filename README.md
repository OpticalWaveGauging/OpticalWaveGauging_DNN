...
1. Run the provided jupyter notebooks (with .ipynb extension) on Google Colab (a free cloud computing service from Google that provides a runtime for executing notebooks, including GPU support). If you are keen to see what this tool looks like, the two provided juypyter notebooks are available through these links: [model training](https://colab.research.google.com/drive/1Uyz2Wsm0pUQWuZ0t6DnBUm4mnmqUERLh) and [model testing](https://drive.google.com/file/d/111c0FWILOcKDwMYkRLAt8GA0s7wx7KyI/view?usp=sharing)

or

2. Set up an anaconda python on your GPU-enabled computer and run the provided python scripts
...
```
python train_OWG.py -c config_nearshore_H.json
```
...
```
python test_OWG_folder.py -i snap_images/data
```
...
```
python gen_ts_from_folder.py -i snap_images/data
```
...
```
python predict_image.py -i snap_images/data/1516401000.cx.snap.jpg
```
